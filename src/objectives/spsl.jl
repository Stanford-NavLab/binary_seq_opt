...
function SPSL(model::Model, prob_data::SubproblemData, iter::Int, X::Matrix{Int})
    ...
    correlation_subset = intersect(Set(correlation_subset), prob_data.variable_correlation_set)
    ...
    @constraint(model, model[:corr][collect(setdiff(prob_data.variable_correlation_set, correlation_subset))] .<= top2psl[1] - 2)
    @constraint(model, -model[:corr][collect(setdiff(prob_data.variable_correlation_set, correlation_subset))] .<= top2psl[1] - 2)
